...
According ot the data distribution chart from data exploration, we observe that the popularity distribution is bi-modal - one approximately normal distribution centered at 40, and one spike distribution for songs with close to zero popularity. However, by looking at the distribution of our predictions, we can see see that our model fails to predict the secondary, large cluster of songs with zero popularity. These failed predictions contributed the the most to our overall MSE.This issue generalizes to the fact that our model fails to accurately predict extremely popular songs, and extremely unpopular songs.
